...
def plot_aggregated_time_series(
    ...
    """
    Plots the time series data for a specific station from Citi Bike data.
    ...
    station_features = features.iloc[row_id]
    ...
    time_series_values = [station_features[col] for col in time_series_columns] + [
        actual_target
    ]

    # Generate corresponding timestamps for the time series
    time_series_dates = pd.date_range(
        start=station_features["pickup_hour"]
        - timedelta(hours=len(time_series_columns)),
        end=station_features["pickup_hour"],
        freq="h",
    )

    # Create the plot title with relevant metadata
    title = f"Pickup Hour: {station_features['pickup_hour']}, Station Name: {station_features['start_station_name']}"
...
def plot_prediction(features: pd.DataFrame, prediction: int):
    ...
    # Create the plot title with relevant metadata
    title = f"Pickup Hour: {pickup_hour}, Station Name: {features['start_station_name'].iloc[0]}"
